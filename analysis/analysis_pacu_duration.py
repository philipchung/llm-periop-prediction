# ruff: noqa: E501
# %% [markdown]
# ### Analysis of Experiment: PACU Phase 1 Duration
#
# Experiment Run:
# python scripts/generate.py --experiment-name=phase1_duration --task=phase1_duration --num-fewshot=5 --num-fewshot=10 --num-fewshot=20 --num-fewshot=50 --num-concurrent=7 --model=gpt-4-1106
# %%
...
from llm.metrics import RegressionMetrics, dummy_regressor_metrics
    ...
    for length_group in p.track(length_groups, task_id=t1):
        ...
        for col in p.track(
            ...
            try:
                ...
                preds = df[col].astype(float)
                targets = df.Label.astype(float)
                boot_metrics_df = RegressionMetrics(
                    preds=preds,
                    targets=targets,
                    name=col,
                ).compute()
...
baseline_boot_metrics = dummy_regressor_metrics(targets=targets, strategy="mean")
...
length_group_comparisons.to_csv(save_dir / "mae_length_group_pvalues.csv")
...
prompt_comparisons.to_csv(save_dir / "mae_prompt_pvalues.csv")
prompt_comparisons
# %% [markdown]
# ### Max Error
# %%
metric_name = "Aggregate/MAE/MaxError"
metric_boot_values = metrics.applymap(lambda df: df.loc[metric_name, "boot_values"])
...
# P-Values comparing Length Groups within each Prompt condition
length_group_comparisons = pd.concat(pval_dfs, axis=0).set_index(["Prompt", "Prompt1", "Prompt2"])
length_group_comparisons.to_csv(save_dir / "maxerror_length_group_pvalues.csv")
length_group_comparisons
# %%
# 2. Pairwise comparison for each prompt, using "all" examples
df = metric_boot_values.loc[:, "all"].to_frame().T
...
wilcoxon_df = pairwise_wilcoxon(df2, correction_method="fdr_bh")
prompt_comparisons = wilcoxon_df.set_index(["Prompt1", "Prompt2"])
prompt_comparisons.to_csv(save_dir / "maxerror_prompt_pvalues.csv")
prompt_comparisons
# %% [markdown]
...
fig, ax = plot_metric_for_prompts(
    ...
    ylabel="Mean Absolute Error (minutes)",
    baseline_value=baseline_value,
)
save_path = plots_save_dir / "mae.png"
fig.savefig(save_path.as_posix())
fig.show()

# Save data required to generate plot
save_pandas(df=boot_mean_long, path=save_dir / "mae_boot_mean_long.csv")
# %% [markdown]
# ### Max Error
# %%
# Get specific metric and all bootstrap values for that metric
...
fig, ax = plot_metric_for_prompts(
    ...
    cot_qa_formatted_prompt_names=prompt_name_df.query("Type == 'CoT'").Formatted.tolist(),
    title="PACU Phase 1 Duration",
    ylabel="Max Error (minutes)",
    baseline_value=baseline_value,
)
save_path = plots_save_dir / "maxerror.png"
fig.savefig(save_path.as_posix())
fig.show()

# Save data required to generate plot
save_pandas(df=boot_mean_long, path=save_dir / "maxerror_boot_mean_long.csv")
# %% [markdown]
...
# %%
# Look at only MAE Metric
metric_name = "Aggregate/MAE/MAE"
mae = mean_ci_for_all_prompts_and_stratifications.loc[metric_name].unstack(-1)
mae = mae.loc[["Baseline"] + prompt_name_df.Formatted.tolist()]
mae.to_csv(save_dir / "mae.csv")
mae
# %%
# Look at only MaxError Metric
metric_name = "Aggregate/MAE/MaxError"
maxerror = mean_ci_for_all_prompts_and_stratifications.loc[metric_name].unstack(-1)
maxerror = maxerror.loc[["Baseline"] + prompt_name_df.Formatted.tolist()]
maxerror.to_csv(save_dir / "maxerror.csv")
maxerror
# %%
